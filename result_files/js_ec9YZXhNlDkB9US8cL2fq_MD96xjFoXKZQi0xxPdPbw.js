...
(function ($) {
  ...
  Drupal.ajax.prototype.commands.ajax_link_redirect = function (ajax, response, status) {
    Drupal.unblockUI();
    if (!Drupal.settings.ajax_links_popup) {
      window.location = Drupal.settings.ajax_links_redirect_url;
    } else {
      tUtils.fancyPdf(Drupal.settings.ajax_links_redirect_url, Drupal.settings.ajax_links_popup_afterclose);
    }
  };

  Drupal.behaviors.ajaxLinksProcessor = {
    attach: function (context) {
      $('.ajax-link').ajaxLinksProcessor();
    }
  };

  $.fn.ajaxLinksProcessor = function () {

    const methods = {
      ajaxLinkOnClick: function () {
        const $link = $(this);

        Drupal.settings.ajax_links_redirect_url = $link.attr("href");
        Drupal.settings.ajax_links_popup = $link.hasClass("popup");

        const popupAfterCloseCallbackName = $link.attr("popupafterclose");
        Drupal.settings.ajax_links_popup_afterclose = popupAfterCloseCallbackName != undefined ? popupAfterCloseCallbackName.split(".").reduce(function (o,n) { return o[n] }, window) : undefined;

        tUtils.blockingAjax($link.attr("ajaxhref"), $link.attr("messageplacement"), false);

        return false;
      }
    };

    const ajaxLinks = this;

    const urlPrefix = Drupal.settings.basePath + Drupal.settings.pathPrefix;
    ajaxLinks.off('click.ajax_links').on('click.ajax_links', methods.ajaxLinkOnClick);
    ajaxLinks.each(function () {
      const link = $(this);
      if (!link.is("[ajaxhref]") || link.attr("ajaxhref") === "") {
        var href = link.attr("href");
        if (href.indexOf(urlPrefix) === 0) {
          href = urlPrefix + "ajax/" + href.substr(urlPrefix.length);
        } else {
          href = "/ajax/" + href;
        }
        link.attr("ajaxhref", href);
      }
    });
...
(function ($) {
  ...
  Drupal.behaviors.responsesListExpanders = {
    attach: function (context, settings) {
      var expanders = $(".request-responses-list-wrapper:not(.mobile) .response.has-services .response-data:first-of-type");
      expanders.addClass('expander');
      expanders.off('click');
      expanders.click(function (e) {
        var parentRow = $(this).closest('[responseId]');
        parentRow.children('.expander').toggleClass('expanded');
        var responseId = parentRow.attr('responseId');
        $('.response-services[responseId="' + responseId + '"]').toggle();

        e.preventDefault();
      });

      $(".request-responses-list-wrapper.mobile .expander, .request-responses-list-wrapper .mobile-expander").click(function (e) {
        var responseId = $(this).attr('responseId');
        $('.response-services[responseId="' + responseId + '"]').show();
        $(this).hide();

        e.preventDefault();
      });

      $('.desktop .expand-services-btn').off('click');
      $('.desktop .expand-services-btn').click(function (e) {
        var btn = $(this);
        if (btn.attr('expanded') === 'true') {
          $('.response-services').hide();
          $('.expander').removeClass('expanded');
          btn.removeAttr('expanded');
          btn.html('Показать исследования');
        } else {
          $('.response-services').show();
          $('.expander').addClass('expanded');
          btn.attr('expanded', true);
          btn.html('Скрыть исследования');
        }
  ...
  Drupal.ajax.prototype.commands.update_services_button = function () {
    var servicesBtn = $('.ajax-services-btn');
    servicesBtn.unbind('click');
    servicesBtn.attr('visible-services', 1);
    servicesBtn.html('Скрыть заказанные услуги');
    servicesBtn.click(function () {
      var visible = $('.ajax-services-btn').attr('visible-services');
      $('.ajax-services-btn').attr('visible-services', visible == 1 ? 0 : 1);
      $('.ajax-services-btn').html(visible == 0 ? 'Скрыть заказанные услуги' : 'Показать заказанные услуги');
      $('#requests-services-info-form').toggle();
      return false;
    });
  };
...
(function ($) {
  ...
  Drupal.behaviors.resultsMessagesHandlers = {
    attach: function (context) {
      $(".expand-request-user-message-panel", context).click(function (e) {
        $(".expand-request-user-message-panel", context).hide();
        $("#request-user-messages-add-panel-wrapper").show();
  ...
  Drupal.ajax.prototype.commands.init_request_messages_links = function (ajax, response) {
    var $link = $(".request-user-messages-link");
    if ($link.hasClass("processed")) {
      return;
    }

    $link.html("Скрыть сообщения").toggleClass("expanded").toggleClass("processed");

    $link.unbind("click").click(function (e) {
      var isVisible = $(".request-user-messages-link").hasClass("expanded");
      $link.html(!isVisible ? "Скрыть сообщения" : "Посмотреть сообщения").toggleClass("expanded");
      $("#request-user-messages-wrapper").toggle();

      e.preventDefault();
    });
  };

  Drupal.ajax.prototype.commands.update_request_messages_form_state = function (ajax, response) {
    $("#request-add-user-message-form").trigger("reset");

    var $link = $(".request-user-messages-link");

    $link.html("Посмотреть сообщения").removeClass("expanded");
    $("#request-user-messages-wrapper").hide();
  };
...
(function ($) {
  ...
  sendRequestResultsDialog = function(email, ajaxRequestUrlCallback) {
    var dialogPane = $('<div id="dialog-pane">');
    var input = $('<input name="email" type="textfield" size=40>');
    input.val(email);
    dialogPane.append(input);
    input.focus();
    ...
    dialogPane.dialog({
      ...
      resizable: false,
      width: 400,
      title: "Введите адрес получателя",
      dialogClass: "send-request-email",
      classes: {"ui-dialog": "send-request-email"},
      buttons: [
        {
          text: "Отправить",
          id: "send-email-btn",
          click: function () {
            ...
            var url = ajaxRequestUrlCallback(email);
            tUtils.blockingAjax(url, 'dialog-pane', false);
          }
        },
        {
          text: "Отказаться",
...
(function ($) {
  ...
  $(document).ready(function () {
    var sendEmailBtn = $("a.email-results-btn");
    if (sendEmailBtn.length > 0) {
      sendEmailBtn.click(function (e) {
        e.preventDefault();
        sendRequestResultsDialog(Drupal.settings.email, function (email) {
          return "/ajax/send_request_email/results/" + Drupal.settings.labId +"/" + Drupal.settings.requestId + "/" + email + "/close_dialog";
        });
      });
    }
  });
